...
public class Main extends TelegramLongPollingBot {
    ...
    public String getBotToken() {
        return "5784653594:AAEI1OZ1ZL9e1B6HkQ-G2bwUT_TACizMyfs";
    }
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasCallbackQuery()) {
            if (update.getCallbackQuery().getData().equals("level_5_task") && getLevel(chatId) == 5){
                setLevel(chatId, 1);

                SendMessage message = createMessage("/start");
                message.setChatId(chatId);
